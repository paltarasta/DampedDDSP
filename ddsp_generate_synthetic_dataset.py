...
import ddsp_synthetic_data  # pylint:disable=unused-import
import gin
import numpy as np
...
class GenerateExampleFn(beam.DoFn):
  ...
  def process(self, seed, generate_fn=ddsp_synthetic_data.generate_notes_v2):
    print('doing process')
    np.random.seed(seed)
...
def _float_dict_to_tfexample(float_dict):
  print('doing float dict to tfexample')
  #print(float_dict)
...
def run():
  ...
  with beam.Pipeline(options=pipeline_options) as pipeline:
    ...
    _ = (
        pipeline
        | beam.Create(np.random.randint(2**20, size=FLAGS.num_examples))
